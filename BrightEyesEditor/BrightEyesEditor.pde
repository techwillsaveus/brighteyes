...
void draw(){
  ...
  pushMatrix();
    ...
    for(int i = 0 ; i < totalFrames; i++){
      ...
      if(((mouseX > x+mapX && mouseX < x+frameSize+mapX)&&(mouseY > y && mouseY < y+frameSize)) && mousePressed) currentFrame = i; 
...
void keyPressed(){
  ...
  if(checkKey(ALT) && checkKey(KeyEvent.VK_C)) arrayCopy(cframe,pframe);//copy frame
  if(checkKey(ALT) && checkKey(KeyEvent.VK_V)) arrayCopy(pframe,cframe);//paste frame
}
void keyReleased(){
  keys[keyCode] = false;
  ...
  if(key == 's') saveToDisk(file == null ? (String)JOptionPane.showInputDialog(frame, "name your creation", "Save BrightEyes animation", JOptionPane.PLAIN_MESSAGE)+".dat" : file);
