...
void setup() {
  ...
  size((int)glasses.width, (int)glasses.height+(int)(glasses.height * .25));
  frameRate(30);
...
void draw() {
  ...
  for (int i = 0 ; i < numLeds; i++) {
    ...
    if (usePreciseClick ? led.contains(mouseX, mouseY) : isOver(led, mouseX, mouseY, px, py)) {
      ...
      if (mousePressed) {
        ...
        else if(mouseButton == RIGHT || erase) cframe[index] = 0;//right click or gui control also erase
  ...
  for (int i = 0 ; i < totalFrames; i++) {
    ...
    if (((mouseX > x+mapX && mouseX < x+frameSize+mapX)&&(mouseY > y && mouseY < y+frameSize*2)) && mousePressed) {
      currentFrame = i;
      gui.timeline.setValue(i);
    } 
  ...
  noStroke();
  fill(map(currentStatusFrames, totalStatusFrames, 0, 255, 32));
...
void mousePressed() {
  prevX = mapX;
  mouseClickX = mouseX;
  if(mouseEvent.getClickCount()==2) gui.toggle();
}
void mouseDragged() {
...
void drawHelp() {
  ...
  help += "File:\n'n' clears everything(new document)/ 's' saves the current document/ drag a .dat file to open an animation\n Double click toggles the controls. '?' toggles this help panel.  Remember to focus the window first. Have fun!";
  fill(255);
